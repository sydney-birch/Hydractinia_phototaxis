...
refblast<-read.table("Homo.fa_ref_blastout_sens_sys_dev",header=F,stringsAsFactors = F)
head(refblast) #syds: 17
names(refblast)[1:2]<-c("gene_acc","seqid")

#link gene_acc to gene symbol (human)
homo_symbols<-read.table("gene_symbol_accid_sens_sys_dev",header=F,stringsAsFactors = F) 
names(homo_symbols)<-c("symbol","gene_acc") #syds: 17
...
all_OG_counts<-read.table("Orthogroups.GeneCount_5-11-20.tsv", header=T, stringsAsFactors = F,row.names=1) 
...
nrow(hydract_acc_apo_OGs_acc_sym)
...
write.table(hydract_acc_apo_OGs_acc_sym, "C:\\Desktop\\R\\Hydractinia_transcriptomics_3-19-20\\Reduced_Hs_sensory_sys_dev_R_output.txt", sep="\t")
