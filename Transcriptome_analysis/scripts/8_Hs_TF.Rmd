...
refblast<-read.table("Homo.fa_ref_blastout_total_tf",header=F,stringsAsFactors = F)
head(refblast) 
names(refblast)[1:2]<-c("gene_acc","seqid")

#link gene_acc to gene symbol (human)
homo_symbols<-read.table("gene_symbol_accid_total_tf",header=F,stringsAsFactors = F) 
...
write.table(hydract_acc_apo_OGs_acc_sym, "C:\\Desktop\\R\\Hydractinia_transcriptomics_3-19-20\\Reduced_Hs_TF_activity_R_output_genes.txt", sep="\t")
